...
Documentation
Doc	Contents
concept.md	Project thesis, core scenario walkthrough, five DAL elements
problem-statement.md	The accountability gap in AI-assisted security workflows
decision-assurance-framework.md	Formal framework with field-level schema tables
architecture-draft.md	Component diagram, data flow, technology choices
threat-model.md	Trust boundaries, 15 threats with mitigations, hardening checklist
user-stories.md	9 user stories — analyst, manager, and auditor workflows
ibm-bob-usage-plan.md	How IBM Bob was used throughout this project
judging-criteria-map.md	Every judging criterion mapped to a specific project artefact
Secondary Milestones (Post-MVP)
Ranked action list with uncertainty scores — expand AI response to a ranked table of N possible actions, each with individual confidence scores
FastAPI / CLI path — expose the DAL engine as a REST API for pipeline integration and automated testing
Jupyter notebook walkthrough — narrative incident walkthrough calling the DAL engine directly, suitable as a submission write-up
Built With
IBM watsonx.ai — Granite model for AI recommendation generation
IBM Cloud Object Storage — Immutable audit record persistence
IBM Bob — Used for planning, documentation, and code implementation throughout
Streamlit — Analyst workbench UI
License
See LICENSE.


A few things worth noting about what changed vs the old version:
- Added the **scenario library table** (all 10 scenarios with severity and MITRE tactic)
- Added a **Built With** section calling out IBM watsonx.ai, IBM COS, and IBM Bob explicitly — good for judges scanning quickly
- The architecture diagram now mentions the fallback behaviour
- Documentation table now links directly to each doc file
